...
def getRsaKeyFile(filename, password=None):
    try:
        ...
        start_adding_honeypots_log("SSH", "Generating new server RSA key.")
...
class SSHHandler(ServerInterface, BaseRequestHandler):
    ...
    def setup(self):
        ...
        start_adding_honeypots_log(
            "SSH", "New request from address {0}.".format(self.client_address)
        )
        try:
            self.transport.load_server_moduli()
        except:
            start_adding_honeypots_log(
                "SSH", "Failed to load moduli -- gex will be unsupported."
            )
            raise
        try:
            ...
            if self.host_key is None:
                start_adding_honeypots_log(
                    "SSH",
                    "Host key not set! SSHHandler MUST define the host_key parameter. Try host_key = paramiko_ssh.getRsaKeyFile('server_rsa.key').",
                )
                raise

        try:
            # Tell transport to use this object as a server
            start_adding_honeypots_log("SSH", "Starting SSH server-side negotiation.")
            self.transport.start_server(server=self)
        except (SSHException) as e:
            start_adding_honeypots_log("SSH", "SSH negotiation failed. {0}.".format(e))
            raise
        ...
        while True:
            ...
            if channel is None:
                ...
                start_adding_honeypots_log(
                    "SSH", "Accepted channel {0}.".format({channel})
                )
    ...
    def set_username(self, username):
        ...
        start_adding_honeypots_log("SSH", "User logged in: {0}.".format(username))
    ...
    def get_allowed_auths(self, username):
        ...
        start_adding_honeypots_log(
            "SSH", "Configured authentication methods: {0}.".format(methods)
        )
        return ",".join(methods)
    ...
    def check_channel_shell_request(self, channel):
        ...
        try:
            ...
            start_adding_honeypots_log(
                "SSH",
                "Requested to start a channel ({0}) that was not previously set up.".format(
                    channel
                ),
            )
            return False
    ...
    def check_channel_pty_request(
        ...
        start_adding_honeypots_log(
            "SSH", "PTY requested. Setting up {0}.".format(self.telnet_handler)
        )
